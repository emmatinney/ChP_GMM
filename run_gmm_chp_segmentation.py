...
#to run on the northeastern discovery cluster: 
#first load anaconda "module load anaconda3"
#then load the environment "source activate surf"
#then load fsl "module load fsl"
#then load freesurfer "module load freesurfer"
#then run the script "python2 run_gmm_chp_segmentation.py freesurfer_dir/ <subid>"
...
cmd = 'fslmaths {subjects_dir}/{subj}/mri/rh_choroid_susan_segmentation.nii.gz -V >> {subjects_dir}/rh.txt'
run_cmd(cmd)

cmd = 'fslmaths {subjects_dir}/{subj}/mri/lh_choroid_susan_segmentation.nii.gz -V >> {subjects_dir}/lh.txt'
run_cmd(cmd)
